...
async def start(client, message):
    ...
    if not files_:
        ...
        try:
            ...
            title = '@MovieupdatewithAK  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), file.file_name.split()))
    ...
    files = files_
    title = '@MovieupdatewithAK  ' + ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@'), files["file_name"].split()))
